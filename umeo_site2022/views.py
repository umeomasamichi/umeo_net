...
#from .models import Good, Message, Stock, Music
...
class MyLoginView(LoginView):
    ...
    template_name = "umeo_site2022/login.html"

class MyLogoutView(LoginRequiredMixin, LogoutView):
    template_name = "umeo_site2022/logout.html"

class IndexView(TemplateView):
    template_name = "umeo_site2022/index.html"
...
class HomeView(TemplateView):
    def get(self, request, *args, **kwargs):
        template_name = "umeo_site2022/home.html"
        ...
        if user.is_anonymous:
            return render(request, "umeo_site2022/index.html")
        ...
        if deltadays.days == 1:
            ...
            if r_days % 10 == 0:
                r_days *= 10
            '''
            return render(request, "umeo_site2022/home.html", {'r_days': r_days, 'flag': 1, "remain":dt.days})
            ...
            user.save()
            return render(request, "umeo_site2022/home.html", {'r_days': user.running_days, 'flag': 0, "remain":dt.days})
...
class UserCreateView(CreateView):
    ...
    template_name = "umeo_site2022/create.html"
    success_url = reverse_lazy("umeo_site2022:login")
...
def UmeoButtonView(request):
    ...
    #    user.save()
    return render(request, 'umeo_site2022/button.html')
...
class BaseView(TemplateView):
    template_name = "umeo_site2022/base.html"

class ExtendsView(TemplateView):
    template_name = "umeo_site2022/extends.html"
...
class CreateMessageView(CreateView):
    ...
    success_url = reverse_lazy('umeo_site2022:message')
...
class RankView(ListView):
    ...
    model = User
    template_name = "umeo_site2022/rank.html"
    ...
    if "number" in request.POST:
        ...
        if id == '100':
            ...
            if user.stock - number >= 0:
                ...
                user.stock -= number
                user.save()
    return render(request, 'umeo_site2022/stock.html', {'now': Stock.objects.all().order_by('-created_at')[0],
...
#   template_name = 'umeo_site2022/stock.html'
...
def UploadView(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('umeo_site2022:home')
    else:
        form = forms.MusicForm()
    return render(request, 'umeo_site2022/model_form_upload.html', {
        'form': form
    })
...
class Upload2View(TemplateView):
    template_name = "umeo_site2022/upload2.html"

class GalleryView(TemplateView):
    template_name = "umeo_site2022/gallery.html"

class TypeView(TemplateView):
    template_name = "umeo_site2022/type.html"
    ...
    def get(self, request, *args, **kwargs):
        ...
        if deltadays.days == 1:
            ...
            user.save()
            r_days = user.running_days
            return render(request, "umeo_site2022/type.html", {'r_days': r_days, 'flag': 1})
        elif deltadays.days > 1:
            #2日以上空けてログインしたら，継続日数を１にして，保存して，投げる
            user.running_days = 1
            user.stock += 1
            ...
            #その日にログインしてたら，ボーナス表示を出さない
            user.save()
            return render(request, "umeo_site2022/type.html", {'r_days': user.running_days, 'flag': 0})
    '''

'''

class SkrollrView(TemplateView):
    template_name = "umeo_site2022/skrollr.html"



class GreetView(FormView):
    template_name = 'umeo_site2022/index2.html'  # テンプレート名(htmlファイル名)
...
class TypingView(ListView):
    model = User
    template_name = "umeo_site2022/typing.html"
    def get_queryset(self):
...
class GoodListView(ListView):
    model = Good
    template_name = "umeo_site2022/good_list.html"
    #templateにはmessage_list.htmlが自動的に選ばれるので，
    #template_nameで上書き．

class GoodCreateView(CreateView):
    model = Good
    template_name = "umeo_site2022/message_form.html"
    form_class = forms.GoodForm
    success_url = reverse_lazy('umeo_site2022:good')
...
class ProfileView(TemplateView):
    template_name =  "umeo_site2022/profile.html"
